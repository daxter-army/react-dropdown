...
function withKeyboardNavigation<P extends ReceivedInputProps>(
  ...
  return (props: Omit<P, keyof InjectedInputProps>) => {
    ...
    const [isMouseHoverAllowed, setIsMouseHoverAllowed] =
    ...
    const scrollToSelectedIndex = (i: number) => {
      ...
      optionsRef.current?.scrollTo(
        0,
        optionItemsRef.current[i]?.current?.offsetTop
      );
    };
    ...
    const onKeyDownHandler = (e: KeyboardEvent) => {
      ...
      if (e.key.toLowerCase() === "arrowup") {
        ...
        // When the items are not visible in the scroll
        else if (scrollDiff < 0) {
          optionsRef.current?.scrollBy(0, scrollDiff - nextElementHeight);
        }
